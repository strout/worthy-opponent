...
impl History {
    fn new() -> History { History { items: Vec::new(), black: None, white: None, none: None } }
    fn from_iter<I>(iter: I) -> History where I: Iterator<Item=Space> { History { items: iter.collect(), black: None, white: None, none: None } }
    fn contains<I>(mut self: &History, iter: I) -> bool where I: Iterator<Item=Space> {
        let mut i = 0;
        for item in iter {
    ...
    fn insert<I>(mut self: &mut History, mut iter: I) -> bool where I: Iterator<Item=Space> {
        loop {
